...
| 动画名称 | 使用位置 | 作用 |
|----------|----------|------|
| `calendarSlideLeft` | line256：星期日期栏切换到下一周 | 日期数字从右侧淡入滑动，模拟日历向后翻页效果 |
| `calendarSlideRight` | line268：星期日期栏切换到上一周 | 日期数字从左侧淡入滑动，模拟日历向前翻页效果 |
| `weekLabelSlideLeft` | line280：顶部周数标签切换到下一周 | 周数文字向左切换时产生轻微进入动画 |
| `weekLabelSlideRight` | line292：顶部周数标签切换到上一周 | 周数文字向右切换时产生轻微进入动画 |
| `dropPulse` | line509：课程拖拽目标格子 | 拖拽课程到目标时间格时，显示向内收缩的脉冲边框，提示可以放置 |
| `courseCardsSlideLeft` | line573：课程卡片周切换 | 切换到下一周后，课程卡片从右侧淡入，增强周视图切换反馈 |
| `courseCardsSlideRight` | line585：课程卡片周切换 | 切换到上一周后，课程卡片从左侧淡入，保持动画方向与操作一致 |
| `cardWiggle` | line597：正在拖拽的课程卡片 | 拖拽中让课程卡片轻微旋转和缩放，强调当前被移动的对象 |
| `backdropFadeIn` | line812：普通弹窗遮罩层 | 添加课程、查询等弹窗打开时，遮罩层平滑淡入 |
| `confirmBackdropIn` | line822：删除确认弹窗遮罩层 | 删除确认弹窗打开时，遮罩层快速淡入，突出确认操作 |
| `modalGrowFromCell` | line832：添加课程弹窗 | 弹窗从被点击的课程格位置放大进入，形成从格子展开表单的视觉效果 |
| `confirmPopIn` | line849：删除确认框 | 删除确认框从下方轻微上浮并放大出现，提升警示弹窗的反馈感 |
| `queryResultsRise` | line1149：查询结果列表 | 查询结果刷新后从下方淡入上浮，提示结果区域已经更新 |
...
|----------------|----------|------|
| `translateY` | line172：周切换圆形按钮 | 鼠标悬停时按钮轻微上浮，提供可点击反馈 |
| `translateX` | line259-300：日期栏与周数标签动画 | 在 `@keyframes` 中动态改变 X 轴位移，实现上一周 / 下一周的方向感 |
| `translate(-50%, -50%)` | line377：空课程格加号提示 | 配合 `top: 50%` 和 `left: 50%`，让添加提示始终在格子中居中 |
| `scale` | line512、line521：拖拽放置脉冲动画 | 通过缩放边框实现目标格的脉冲提示，强化拖拽放置状态 |
| `translateY` | line550：课程卡片悬停 | 鼠标悬停时卡片向上移动 1px，配合阴影形成上浮效果 |
| `translateX` | line576-593：课程卡片周切换动画 | 课程卡片根据切换方向从左侧或右侧进入，使周视图切换更自然 |
| `rotate` + `translateX` + `scale` | line599-607：拖拽中的课程卡片 | 通过旋转、左右位移和缩小组合，形成拖拽时的轻微晃动反馈 |
| `translate` + `scale` | line835-845：课程编辑弹窗 | 根据点击格子计算起始位移，让弹窗从对应格子缩放展开 |
| `translateY` + `scale` | line852-857：删除确认框 | 弹窗从下方向上出现并恢复到正常大小，提升确认操作的层次感 |
| `translateY` | line1152-1157：查询结果动画 | 查询结果区域从下方进入，突出搜索后的内容变化 |
| `translateY` + `rotate` | line1211：周数选择模式对勾 | 使用边框和旋转绘制选中状态的对勾图形 |
...
| 断点 / 技术 | 使用位置 | 作用 |
|-------------|----------|------|
| `@media (max-width: 1180px)` | line1236：中等屏幕适配 | 调整页面主体宽度、课程表列宽和弹窗宽度，使页面在较窄桌面窗口中仍保持完整布局 |
| `@media (max-width: 900px)` | line1261：平板与手机横屏适配 | 取消页面固定最小宽度，允许页面纵向滚动，并把头部按钮改为两列网格布局 |
| 横向滚动课程表 | line1309-1331：课程表外层和表格宽度 | 小屏幕下为 `.week-panel` 开启 `overflow-x: auto`，同时给 `.schedule-board` 设置固定宽度，避免课程格被压缩变形 |
| 粘性时间轴 | line1334-1348：月份格与左侧节次列 | 使用 `position: sticky` 固定左侧月份和节次，横向滚动课程表时仍能看清每一行对应的时间 |
| 移动端删除按钮 | line1358：课程卡片删除按钮 | 在无鼠标悬停的设备上让删除按钮保持显示，解决触屏设备无法 hover 的问题 |
| 移动端弹窗宽度 | line1367-1375、line1428-1431：课程编辑、周数选择、确认弹窗 | 弹窗宽度改为接近视口宽度，并限制最大高度，避免表单内容溢出屏幕 |
| `@media (hover: none)` | line1379：触屏设备交互提示 | 在触屏设备上直接显示空课程格的浅色背景和加号提示，弥补没有鼠标悬停的问题 |
| `@media (max-width: 600px)` | line1390：手机窄屏适配 | 缩小标题、按钮、弹窗内边距和查询控件尺寸，让表单在窄屏下仍然清晰可读 |
| 表单单列布局 | line1455-1473：移动端添加课程表单 | 将原本左右两列的表单改为单列排列，输入框左对齐，避免文字和输入内容挤在一起 |
| 周数选择器压缩 | line1480-1503：移动端周数弹窗 | 允许单周、双周、全选按钮换行，并缩小周数按钮高度和字号，提升手机端可操作性 |
